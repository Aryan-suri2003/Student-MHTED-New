...
const navigation: NavGroup[] = [
  { 
    ...
    items: [
      ...
      { name: 'Examination & Result', href: '/students?tab=examination', icon: FileText },
        ...
        <div key={idx}>
          ...
          <div className="flex flex-col gap-1.5">
            {group.items.map((item) => {
              ...
              const isExactActive = itemTab
                ? pathname === item.href.split('?')[0] && currentTab === itemTab && !item.subItems
                : pathname === item.href && !item.subItems;

              const isParentActive = itemTab
                ? isExactActive
                : pathname === item.href || (item.subItems && isStudentsPage && item.name === 'Students' && currentTab !== 'examination');
